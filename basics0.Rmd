...
output:
  html_document:
    toc: true
    toc_depth: 3  # Adjust the depth of the TOC (number of heading levels to include)
    toc_float: true  # Optional: makes the TOC floating
...
- One of the great features of RStudio is that you can customize the layout by reorganizing these windows to suit your workflow.
...
Use the code editor if you want to develop more complex, reusable, and maintainable code that can be saved and executed later. 
- We won't be working in the code editor at this level. 
- It will be introduced at the beginning of the Intermediate level workshop. <br>

### Working in the Console

The console is a lot like working in Terminal (mac) or Command Prompt/PowerShell (PC). 
- Each new command line begins with the angle bracket `>` also known as the ‘prompt’ symbol. <br>

You will type the commands into the Console after the most recent angle bracket `>` also known as the ‘prompt’ symbol. 
...
-   Sometimes the output can be extensive and show more information than you expected
  - E.g., when you load in a package (we will discuss packages more in Activity 3). <br>
...
<br> `help("vector")` will provide you with information about the mean function in RStudio. 
- The help information will be displayed in the Console following your command.
...
[Task 2.1.1:]{.underline} Create a variable for a pig's first name. 
...
[Task 2.1.2:]{.underline} Create a variable for a Bart's last name. 
...
[Task 2.1.4:]{.underline} Create a variable for Bart's height as a piglet.
...
[Task 2.1.5:]{.underline} Create a variable for Bart's height now.
...
[Task 2.1.7:]{.underline} **REWORK** default *pig1.size* Create two variables (pig1.mini and pig1.large) denoting that Bart is a large pig.
...
[Task 2.2.1:]{.underline} Make a vector for the following weight values of miniature goats. Name your variable 'goat.weight'
...
- the goat.weight vector is now listed in the Environment tab (top right window) under [Values]{.underline}.<br>
...
[Task 2.2.2:]{.underline} Display (aka 'print') the contents of the vector containing the goat weights.
...
<br>
[Task 2.2.4:]{.underline} Make a vector for the following name values of miniature goats. Name your variable 'goat.name'
...
- For more on matrices, [check me out](https://www.w3schools.com/r/r_matrices.asp){:target="\_blank"}.
...
The function names for the following three statistical measures (mean, median, standard deviation) are quite intutive. 
- It is just the name or abbreviation of the measure, 
  - where the argument is the object containing the set of values we are analyzing. - Each function takes the vector as its argument.
...
[Task 4.1.3:]{.underline} Import the dataset of Income data
...
We can see in the histogram that there are 7 intervals with equally spaced breaks. 
In this case, the height of a cell is equal to the number of observations falling in that cell. 
...
hist(income$experience, breaks=4)
```
