...
<!--# Are you going to have them knit this beforehand? how will they be reading it? I think that needs to explained thoroughly -->
...
<!--# I would add the images again here for each of these and introduce in the presentation portion of the workshop -->
...
3.  *Files/Plots/Packages/Help Pane:*
    ...
    -   F*iles:* Browse, open, and manage files in your working directory.
...
You will type the commands into the Console after the most recent angle bracket `>` also known as the ‘prompt’ symbol. When you are ready to execute ('run') the command, press the ‘enter’ or ‘return’ key on your keyboard. The output to the command will appear below your command.

<!--# I would suggestusing point form whenever possible. I like how you did the definitions in the above section -->

*command line:* lines of code in your console

*‘prompt’ symbol* `>` : Each command line starts with this
...
For all tasks in this workshop, enter your commands in the Console (top tabs say "console". "terminal", and "background jobs".<br>
...
**Definition - "Function":** A set of instructions defined to perform a specific task. <!--# example(s)? -->

**Definition - "Function Call":** The act of executing a function with specific arguments, if required, to produce a result. <br> <!--# example(s)? -->
...
We have just looked at vectors, which contain variables of the same type. Now we'll look at different types of variables. <!--# I would move this section above the vector section since you talk about variables before this, and this term might be unclear for some people-->
...
[Task 2.2.1:]{.underline} Create a variable for a pig's first name. `The first pig's first name is 'Bart'.`
...
[Task 2.2.2:]{.underline} Create a variable for a Bart's last name. `Bart's last name is 'Smith'.`
...
[Task 2.2.4:]{.underline} Create a variable for Bart's height as a piglet: 10.
...
[Task 2.2.5:]{.underline} Create a variable for Bart's height now: 13.5.
...
<summary>Expand to view the list of Environment Variables</summary>
...
We can also execute a **'summary'** to generate several descriptive statistics at the same time.
...
<summary>Show Codeand Histogram</summary>
...
-   ?? In **Yes** to install the “**readxl**” package.
    ...
    <!--# Move this to where it is relevant to the activity and add text explanation-->

    ```{r setup, include=FALSE}
    knitr::opts_chunk$set(echo = TRUE) 
    #uncomment the following if you haven't installed and loaded the 'readxl' package 
    #install.packages("readxl") 
    #library(readxl)
    ```

*Note:* Don't worry about making a mistake importing this data. You can always remove it using the `rm()` function. <!--# Example using rm()? Otherwise I think this might be confusing -->
...
*Additional:* If you preferred having 4 intervals (i.e., 'bins'), use can set that using the `breaks=''` parameter. <!--# might have missed it, but did you define "parameter"? -->
