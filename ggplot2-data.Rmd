...
### 2.1. Scatter Plots
First things first, we need to quickly clean up our dataframe for scatter plots. Copy and paste the following code into your console, and execute.
<br>
```{r}
...
<br>
```{r}
#create a column (vector) with just the first word of the bean_type() cell, where the second parameter of word() is the number of words to include in the new value.
chocolateData$bean_type_simplified <- word(chocolateData$bean_type, 1)

#remove punctuation using gsub() by globally substituting any punctuation using the regex (regular expression) '[[:punct:]]' with nothing '', in the vector where the replacements will be made: chocolateData$bean_type_simplified.
chocolateData$bean_type_simplified <- gsub('[[:punct:]]', '', chocolateData$bean_type_simplified)

#create (not add) a vector of the names of beans that occur in the bean_type_simplified vector of our dataset more than 20 times.
...
#create a subset (a new dataset) only containing the chocolate bars made using common beans
